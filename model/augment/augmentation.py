import numpy as np
import model.augment.dtw as dtw
import torch

augmentations = {
    'origin': 'origin(x)',
    'jitter': 'jitter(x, args.aug_jitter_sigma)', 
    'scaling': 'scaling(x, args.aug_scale_sigma)', 
    'rotation': 'rotation(x)',
    'permutation': 'permutation(x, args.aug_permu_segments, args.aug_permu_seg_mode)', 
    'magnitude_warp': 'magnitude_warp(x, args.aug_magwarp_sigma, args.aug_magwarp_knot)',
    'time_warp': 'time_warp(x, args.aug_time_warp_sigma, args.aug_time_warp_knot)',
    'window_slice': 'window_slice(x, args.aug_window_slice_ratio)',
    'window_warp': 'window_warp(x, args.aug_window_warp_ratio, args.aug_window_warp_scale)',
    'spawner': 'spawner(x, labels, args.aug_jitter_sigma)',
    'wdba': 'wdba(x, labels, args.aug_batch_size, args.aug_slope_constraint, args.aug_use_window)',
    'random_guided_warp_shape': 'random_guided_warp_shape(x, labels, args.aug_slope_constraint, args.aug_use_window)', 
    'discriminative_guided_warp_shape': 'discriminative_guided_warp_shape(x, labels, args.aug_batch_size, args.aug_slope_constraint, args.aug_use_window)',
    'random_guided_warp': 'random_guided_warp(x, labels, args.aug_slope_constraint, args.aug_use_window)', 
    'discriminative_guided_warp': 'discriminative_guided_warp(x, labels, args.aug_batch_size, args.aug_slope_constraint, args.aug_use_window)', 
}

def load_aug(name, x, labels, args):
    assert name in augmentations.keys()
    x = x.cpu().numpy()
    labels = labels.cpu().numpy()

    return eval(augmentations[name])


def rand_aug(x, labels, args):
    aug_list = list(augmentations.keys())
    idx = torch.randint(len(aug_list), (2,))
    chosen_aug1 = aug_list[idx[0]]
    chosen_aug2 = aug_list[idx[1]]
    x = x.cpu().numpy()
    labels = labels.cpu().numpy()
    aug1 = eval(augmentations[chosen_aug1])
    aug2 = eval(augmentations[chosen_aug2])
    return aug1, aug2
...
def window_warp(x, window_ratio=0.1, scales="[0.5,2.]"):
    # https://halshs.archives-ouvertes.fr/halshs-01357973/document
    warp_scales = np.random.choice(eval(scales), x.shape[0])
...
def wdba(x, labels, batch_size=6, slope_constraint="symmetric", use_window=1, verbose=-1):
...
def random_guided_warp(x, labels, slope_constraint="symmetric", use_window=1, dtw_type="normal", verbose=-1):
    # use verbose = -1 to turn off warnings
    # slope_constraint is for DTW. "symmetric" or "asymmetric"
...
def random_guided_warp_shape(x, labels, slope_constraint="symmetric", use_window=1):
    return random_guided_warp(x, labels, slope_constraint, use_window, dtw_type="shape")

def discriminative_guided_warp(x, labels, batch_size=6, slope_constraint="symmetric", use_window=1, dtw_type="normal", use_variable_slice=1, verbose=-1):
    # use verbose = -1 to turn off warnings
    # slope_constraint is for DTW. "symmetric" or "asymmetric"
    # dtw_type is for shapeDTW or DTW. "normal" or "shape"
...
def discriminative_guided_warp_shape(x, labels, batch_size=6, slope_constraint="symmetric", use_window=1):
